  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="mb-6 bg-white p-6 rounded shadow">
        <select value={idBarang} onChange={(e) => setIdBarang(e.target.value)} className="w-full p-2 mb-2 border rounded" required>
        ...
        <select value={jenis} onChange={(e) => setJenis(e.target.value)} className="w-full p-2 mb-2 border rounded">
          <option value="masuk">Masuk</option>
          <option value="keluar">Keluar</option>
        </select>

        <input type="number" placeholder="Jumlah" className="w-full p-2 mb-2 border rounded" value={jumlah} onChange={(e) => setJumlah(e.target.value)} required />
        <input type="text" placeholder="Keterangan" className="w-full p-2 mb-4 border rounded" value={keterangan} onChange={(e) => setKeterangan(e.target.value)} />

        <button type="submit" disabled={loading} className={`w-full py-2 rounded text-white ${loading ? "bg-gray-400" : "bg-green-500 hover:bg-green-600"}`}>
          ...
          <tbody>
            {transaksi.map((t) => (
              <tr key={t.id} className="border-b">
                ...
                <td className="p-2 border">{t.barang?.nama || t.id_barang}</td>
